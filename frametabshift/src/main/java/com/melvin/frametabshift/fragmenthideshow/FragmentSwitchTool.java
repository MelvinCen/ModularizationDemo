		...
		for (int i = 0; i < clickableViews.length; i++) {
			if (v.getId() == clickableViews[i].getId()) {
				...
				if (showAnimator) {
					...
					if (i > exitIndex){
						fragmentTransaction.setCustomAnimations(R.animator.tab_shift_slide_right_in, R.animator.tab_shift_slide_left_out);
					} else if (i < exitIndex) {
						fragmentTransaction.setCustomAnimations(R.animator.tab_shift_slide_left_in, R.animator.tab_shift_slide_right_out);
					}
				}
