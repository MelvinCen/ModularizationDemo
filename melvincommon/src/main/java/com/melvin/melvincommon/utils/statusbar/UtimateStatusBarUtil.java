package com.melvin.melvincommon.utils.statusbar;
...
public class UtimateStatusBarUtil {
    ...
    public void setColor(@ColorInt int color, int alpha){
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            //这句话千万不要忘记，不然画systemBar的背景是无效的，还有xml布局中的fitSystemWindow和clipToPadding
            window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
...
//                    | WindowManager.LayoutParams.FLAG_TRANSLUCENT_NAVIGATION);
    ...
    /**
     *目的等同与setImmersionBar方法，透明状态栏和导航栏，当上面的方法不管用时可以尝试这个方法,测试在华为荣耀中可行
     * @param activity
     */
